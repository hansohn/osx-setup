...
if [ ! -f "${HOME}/Library/Colors/solarized.clr" ] ; then
  ...
  wget -O "${HOME}/Library/Colors/solarized.clr" https://raw.githubusercontent.com/altercation/solarized/master/apple-colorpalette-solarized/solarized.clr;
fi
...
if [[ ! $(defaults read "${HOME}/Library/Preferences/com.apple.Terminal.plist" "Default Window Settings") == "solarized-light" ]] ; then
  echo "==> Setting solarized-dark as default color profile in osx terminal";
  defaults write "${HOME}/Library/Preferences/com.apple.Terminal.plist" "Default Window Settings" "solarized-light";
  defaults write "${HOME}/Library/Preferences/com.apple.Terminal.plist" "Startup Window Settings" "solarized-light";
fi
